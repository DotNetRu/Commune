...
namespace DotNetRu.MeetupManagement.WebApi.Contract.Filters
{
    ...
    public class BasePathFilter : IDocumentFilter
    {
        ...
        public void Apply(SwaggerDocument swaggerDoc, DocumentFilterContext context)
        {
            ...
            var pathsToModify = swaggerDoc.Paths.Where(p => p.Key.StartsWith(this.BasePath, StringComparison.Ordinal)).ToList();

            foreach (var path in pathsToModify)
            {
                if (path.Key.StartsWith(this.BasePath, StringComparison.Ordinal))
                {
